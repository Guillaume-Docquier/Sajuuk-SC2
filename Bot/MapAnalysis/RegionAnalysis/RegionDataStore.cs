...
public class RegionDataStore {
    ...
    public RegionDataStore(ITerrainTracker terrainTracker) {
    ...
    public void Save(string mapName, RegionsData regionsData) {
        ...
        var jsonString = JsonSerializer.Serialize(regionsData, JsonSerializerOptions);
        File.WriteAllText(jsonSaveFilePath, jsonString);

        SaveAsImage(mapName, regionsData.Regions);
    }

    public RegionsData Load(string mapName) {
        if (!IsEnabled) {
            return null;
        }
        ...
        return JsonSerializer.Deserialize<RegionsData>(jsonString, JsonSerializerOptions)!;
    }
